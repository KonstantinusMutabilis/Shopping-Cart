...
if (!userPermissions.has('ADD_TO_CART')) {
    document.querySelectorAll('.add-to-cart').forEach(elem => elem.disabled = true);
}

if (!userPermissions.has('REMOVE_FROM_CART')) {
    document.querySelectorAll('.remove-from-cart').forEach(elem => elem.disabled = true);
}

if (!userPermissions.has('ADD_TO_CART') && !userPermissions.has('REMOVE_FROM_CART')) {
    document.querySelectorAll('.cart-action').forEach(elem => elem.disabled = true);
}
    ...
    if (userPermissions.has('DISCOUNT_15')) {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td class="text-right">-₪${Number((totalAmount.innerText.slice(1)) * 0.15).toFixed(2)}</td>
            </tr>
        `;
        totalAmount.innerText = `₪${Number((totalAmount.innerText.slice(1)) * 0.85).toFixed(2)}`;
    }
}